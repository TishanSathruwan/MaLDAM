...
import utils.val as validate  # for end-of-epoch mAP
...
from utils.torch_utils import (EarlyStopping, ModelEMA, de_parallel, select_device, smart_DDP, smart_optimizer2,
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if RANK == -1 and batch_size == -1:  # single-GPU only, estimate best batch size
        batch_size = check_train_batch_size(model, imgsz, amp)/3 # for contrastive training 3x inputs
    ...
    if opt.sync_bn and cuda and RANK != -1:
        ...
        projection_head = torch.nn.SyncBatchNorm.convert_sync_batchnorm(projection_head).to(device)
    ...
    # Process 0
    if RANK in {-1, 0}:
        val_loader = create_val_dataloader(val_path,
                                       imgsz,
    ...
    if cuda and RANK != -1:
        ...
        projection_head = smart_DDP(projection_head)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs1, targets, paths, _, mh, ml) in pbar:  # batch -------------------------------------------------------------
            ...
            imgs_hcm = imgs_hcm.to(device, non_blocking=True).float() / 255  # uint8 to float32, 0-255 to 0.0-1.0
            imgs_lcm = imgs_lcm.to(device, non_blocking=True).float() / 255  # uint8 to float32, 0-255 to 0.0-1.0
            ...
            # Multi-scale- NOT SURE WHAT TO DO FOR HCM AND LCM IMAGES
            if opt.multi_scale:
            ...
            #                         (f'{epoch}/{epochs - 1}', mem, f'DAC {mdac_loss:.4g}', *mloss, targets.shape[0], imgs.shape[-1]))
            #     callbacks.run('on_train_batch_end', model, ni, imgs, targets, paths, list(mloss) + [mdac_loss])
            #     if callbacks.stop_training:
            #         return
        ...
        if RANK in {-1, 0}:
            ...
            if (not nosave) or (final_epoch and not evolve):  # if save
                ckpt = {
                    ...
                    'projection_head': deepcopy(de_parallel(projection_head)).half(),
...
def main(opt, callbacks=Callbacks()):
    ...
    if opt.resume and not check_comet_resume(opt) and not opt.evolve:
        ...
        opt.data, opt.cfg, opt.hyp, opt.weights, opt.project = \
            check_file(opt.data), check_file(opt.cfg), check_yaml(opt.hyp), str(opt.weights), str(opt.project)  # checks
...
# python train_contrastive.py --data m5.yaml --epochs 40 --weights yolov5m.pt --img 640 --cfg yolov5m.yaml --cache disk  --batch-size 32 2>&1 | tee ssl1.out

# python train_contrastive.py --data m5.yaml --epochs 40 --weights yolov5m.pt --img 640 --cfg yolov5m.yaml --cache disk  --batch-size 32 --hyp data/hyps/hyp_ours.yaml 2>&1 | tee ssl2_hypours.out

# python train_contrastive.py --data m5.yaml --epochs 40 --weights yolov5m.pt --img 640 --cfg yolov5m.yaml --cache disk  --batch-size 32 --hyp data/hyps/hyp_ours.yaml 2>&1 | tee ssl2_hypours.out

# python train_contrastive.py --data m5.yaml --epochs 40 --weights yolov5m6.pt --img 1280 --cfg yolov5m.yaml --cache disk  --batch-size 32 --device 0,1 2>&1 | tee ssl3_1280_bs32.out



# python train_contrastive.py --data m5.yaml --epochs 100 --weights yolov5m.pt --img 640 --cfg yolov5m.yaml --cache disk  --batch-size 32 2>&1 | tee ssl3_100epochs.out 

# python train_contrastive.py --data m5.yaml --epochs 40 --weights yolov5m.pt --img 640 --cfg yolov5m.yaml --cache disk  --batch-size 32 --optimizer AdamW --hyp params3.yaml 2>&1 | tee ssl4_adamW_params3.out 


#python train_contrastive.py --data m5.yaml --epochs 40 --weights yolov5m.pt --img 640 --cfg yolov5m.yaml --cache disk  --batch-size 32 --optimizer AdamW --hyp params3.yaml  --cos-lr 2>&1 | tee ssl5_adamW_params3_cosine.out 
